...
class CGoal:public CSceneX
{
	//公開メンバ
	public:
		CGoal(OBJTYPE nObjType,int nPriority=3);		//コンストラクタ
		~CGoal();										//デストラクタ
		...
		static CGoal* CGoal::Create(float fPosX,float fPosY,float fPosZ,CRenderer* m_pRenderer);	//生成
	...
	private:
		...
		static CRenderer *m_pGoal;			//目的地のポインタ
